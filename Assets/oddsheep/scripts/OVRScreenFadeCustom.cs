...
************************************************************************************/
/**
 * Customised by Xavier Selva - Odd Sheep SL
 **/
...
public class OVRScreenFadeCustom : MonoBehaviour
{
    ...
    /// Start a fade out
    /// </summary>
    public void FadeOut(FadeFinished fadeFinished = null, string fadeFinishedMsg = null)
    {
#if UNITY_STANDALONE || UNITY_EDITOR
        if (fadeFinished != null)
            fadeFinished(fadeFinishedMsg);
#else
        StartCoroutine(Fade(0, 1, fadeFinished, fadeFinishedMsg));
#endif
    }
    ...
    public bool isFaded()
    {
        return isActiveAndEnabled && currentAlpha == 1;
    }
    /// <summary>
    ...
    public delegate void FadeFinished(string detail);
    /// <summary>
    ...
    private void SetMaterialColor(Color toColor, float amount)
    {
        Color c = fadeColor;
        //Color color = Mathf.Max(Color.Lerp(c, toColor, currentAlpha), uiFadeAlpha);
        Color color = Color.Lerp(c, toColor, amount);
        ...
        if (fadeMaterial != null)
        {
            ...
            fadeRenderer.material = fadeMaterial;
            fadeRenderer.enabled = isFading;
            //Debug.Log("ColorChange to " + color + "("+toColor+")");
        }
    }
    /*
    float oscillateCount = 0;
    internal void oscillate()
    {
        //Debug.Log("Fading? " + isFading);
        if (false && isFading)
        {
            //Debug.Log("Fading " + oscillateCount);
            SetMaterialColor(Color.magenta, oscillateCount);
            oscillateCount = Mathf.Sin(Time.time);
        }
